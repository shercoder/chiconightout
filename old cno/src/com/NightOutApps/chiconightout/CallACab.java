...
public class CallACab {
	...
	private static final int entries = 4;	// made it static final because ArrayList needs static reference
	private static ArrayList<Integer> tries = new ArrayList<Integer>(entries);
//    private static int currentPhoneNum = 0;
	private static int dialCounter = 0;	// to keep track how many times user has dialed
	...
	public void launchDialer(String number, Context context){
        ...
        context.startActivity(new Intent(Intent.ACTION_DIAL, Uri.parse(numberToDial)));
    }
	
	/**
	 * Generates random number between 0(inclusive) and number of phone entries (exclusive)
	...
	private int randomNum() {
		...
		while(!found) {
			...
			// Adds the random number to ArrayList if it does not exist in the list
